...
下図のようなイメージで、リザルト画面の名前の部分を囲むようにカーソルを移動してください。
...
- **履歴:**
  http://127.0.0.1:8888/history で過去のスクリーンショットを、閲覧可能。
- **編集:**
  http://127.0.0.1:8888/edit で、もしタグの誤認や点数の修正が必要な場合、Web から編集可能。
...
- ユーザーに「どれが余分ですか？」と聞かれますので、例えば、「3」と入力したら、3 番目の行（ここでは 'F Bunty'）が削除できます。
- 削除後、最終的に 12 行に調整します。
...
- OCR 結果として検出された行数が 12 行未満の場合も、現在の行一覧が表示されます。
...
以上でセットアップが完了すれば、マリオカート 8 デラックス のラウンジの集計を自動化できます。
